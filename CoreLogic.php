...
function EvaluateCombatChain(&$totalAttack, &$totalDefense, &$attackModifiers = [])
{
  ...
  for ($i = 1; $i < count($combatChain); $i += CombatChainPieces()) {
    ...
    if ($combatChain[$i] == $mainPlayer) {
      if ($i == 1)
        $attack = $combatChainState[$CCS_LinkBaseAttack];
      else
  ...
  if ($combatChainState[$CCS_WeaponIndex] != -1) {
    $attack = 0;
    if ($attackType == "W")
      $attack = $mainCharacter[$combatChainState[$CCS_WeaponIndex] + 3];
    else if (DelimStringContains(CardSubtype($combatChain[0]), "Aura"))
      $attack = $mainAuras[$combatChainState[$CCS_WeaponIndex] + 3];
    else if (IsAlly($combatChain[0])) {
      $allies = &GetAllies($mainPlayer);
      if (count($allies) > $combatChainState[$CCS_WeaponIndex] + 7)
...
{
  global $combatChain;
  if ($amount > 0 && $combatChain[0] == "OUT100")
    $amount += 1;
  if ($amount > 0 && ($combatChain[0] == "OUT065" || $combatChain[0] == "OUT066" || $combatChain[0] == "OUT067") && ComboActive())
    $amount += 1;
  if ($amount > 0)
    $amount += PermanentAddAttackAbilities();
  $totalAttack += $amount;
}
...
function CacheCombatResult()
{
  ...
  if (count($combatChain) > 0) {
    ...
    if ($combatChainState[$CCS_CachedNumBlockedFromHand] == 0)
...
function HasIncreasedAttack()
{
  ...
  if (count($combatChain) > 0) {
    ...
    if ($attack > AttackValue($combatChain[0]))
      return true;
  }
  return false;
}
...
function DealDamageAsync($player, $damage, $type = "DAMAGE", $source = "NA")
{
  ...
  if ($type == "COMBAT" && $damage > 0 && EffectPreventsHit())
...
function FinalizeDamage($player, $damage, $damageThreatened, $type, $source)
{
  ...
  if ($damage > 0) {
    ...
    if ($player == $defPlayer && $type == "COMBAT" || $type == "ATTACKHIT")
      $combatChainState[$CCS_AttackTotalDamage] += $damage;
...
{
  global $currentTurnEffects;
  if (CardType($source) == "AA" && (SearchAuras("CRU028", 1) || SearchAuras("CRU028", 2)))
    return;
  for ($i = count($currentTurnEffects) - CurrentTurnPieces(); $i >= 0; $i -= CurrentTurnPieces()) {
    if ($currentTurnEffects[$i + 1] == $target)
      continue;
    if ($type == "COMBAT" && HitEffectsArePrevented())
      continue;
    $remove = 0;
    switch ($currentTurnEffects[$i]) {
...
function PlayerWon($playerID)
{
  ...
  $winner = $playerID;
  if ($playerID == 1 && $p1uid != "")
    WriteLog($p1uid . " wins!", $playerID);
  elseif ($playerID == 2 && $p2uid != "")
    WriteLog($p2uid . " wins!", $playerID);
  else
    WriteLog("Player " . $winner . " wins!");

  $inGameStatus = $GameStatus_Over;
  $turn[0] = "OVER";
  SetCachePiece($gameName, 14, 6);//$MGS_GameOverStatsLogged
  if (GetCachePiece($gameName, 14) == 7)
    return;//$MGS_StatsLoggedIrreversible
  try {
...
function CombatChainClosedMainCharacterEffects()
{
  ...
  for ($i = 0; $i < count($chainLinks); ++$i) {
    for ($j = 0; $j < count($chainLinks[$i]); $j += ChainLinksPieces()) {
      if ($chainLinks[$i][$j + 1] != $mainPlayer)
        continue;
    ...
    for ($j = 0; $j < count($chainLinks[$i]); $j += ChainLinksPieces()) {
      ...
      switch ($chainLinks[$i][$j]) {
        ...
        case "RVD003":
          ...
          $rv = "";
          if (count($deck) == 0)
            $rv .= "Your deck is empty. No card is revealed.";
...
function NumDefendedFromHand() //Reprise
{
  ...
  for ($i = 0; $i < count($combatChain); $i += CombatChainPieces()) {
    if ($combatChain[$i + 1] == $defPlayer) {
      ...
      if ($type != "I" && $combatChain[$i + 2] == "HAND")
        ++$num;
    }
  }
  return $num;
...
function NumBlockedFromHand() //Dominate
{
  ...
  for ($i = 0; $i < count($layers); $i += LayerPieces()) {
    ...
    if ($params[0] == "HAND" && CardType($layers[$i]) == "DR")
      ++$num;
  }
  return $num;
}
...
function PlayerHasFewerEquipment($player)
{
  ...
  for ($i = 0; $i < count($thisChar); $i += CharacterPieces()) {
    if ($thisChar[$i + 1] != 0 && CardType($thisChar[$i]) == "E")
      ++$thisEquip;
  }
  for ($i = 0; $i < count($thatChar); $i += CharacterPieces()) {
    if ($thatChar[$i + 1] != 0 && CardType($thatChar[$i]) == "E")
      ++$thatEquip;
  }
...
function RollDie($player, $fromDQ = false, $subsequent = false)
{
  ...
  if ($GGActive) {
    if ($fromDQ && !$subsequent)
      PrependDecisionQueue("AFTERDIEROLL", $player, "-");
    GamblersGloves($player, $player, $fromDQ);
    GamblersGloves(($player == 1 ? 2 : 1), $player, $fromDQ);
    if (!$fromDQ && !$subsequent)
      AddDecisionQueue("AFTERDIEROLL", $player, "-");
  } else {
    if (!$subsequent)
      AfterDieRoll($player);
  }
...
function AfterDieRoll($player)
{
  ...
  if ($roll > GetClassState($player, $CS_HighestRoll))
    SetClassState($player, $CS_HighestRoll, $roll);
}
...
function IsCharacterAbilityActive($player, $index, $checkGem = false)
{
  ...
  if ($checkGem && $character[$index + 9] == 0)
    return false;
...
function GetMultizoneIndicesForTitle($player, $title, $onlyReady = false)
{
  ...
  if ($allies != "") {
    ...
    for ($i = 0; $i < count($allies); ++$i) {
      ...
      if (!$onlyReady || !$ally->IsExhausted())
        array_push($indices, "MYALLY-$allies[$i]");
    }
  }
...
{
  if (SearchCurrentTurnEffects("UPR187", $player))
    return true;//Erase Face
  return false;
}
...
function NameOverride($cardID, $player = "")
{
  ...
  if (SearchCurrentTurnEffects("OUT183", $player))
    $name = "";
  return $name;
}
...
{
  if (!$cardID || $cardID == "" || strlen($cardID) < 3)
    return "";
...
function TraitContainsAny($cardID, $traits, $player = "", $index = -1)
{
  ...
  for ($i = 0; $i < count($traitsArr); $i++) {
    if (TraitContains($cardID, $traitsArr[$i], $player, $index))
      return true;
  }
  return false;
}
...
function TraitContainsAll($cardID, $traits, $player = "", $index = -1)
{
  $traitsArr = explode(",", $traits);
  for ($i = 0; $i < count($traitsArr); $i++) {
    if (!TraitContains($cardID, $traits[$i], $player, $index))
      return false;
  }
  return true;
}
...
function HasKeyword($cardID, $keyword, $player = "", $index = -1)
{
  switch ($keyword) {
    ...
    case "Raid":
      return RaidAmount($cardID, $player, $index, true) > 0;
    case "Grit":
      return HasGrit($cardID, $player, $index);
    case "Restore":
      return RestoreAmount($cardID, $player, $index) > 0;
    case "Bounty":
      return CollectBounty($player, $cardID, $cardID, false, $player, true) > 0; // Since we don't have information about "exhausted" and "owner," this data may be imprecise in very rare cases.
    case "Overwhelm":
      return HasOverwhelm($cardID, $player, $index);
    case "Saboteur":
      return HasSaboteur($cardID, $player, $index);
    case "Shielded":
      return HasShielded($cardID, $player, $index);
    case "Sentinel":
      return HasSentinel($cardID, $player, $index);
    case "Ambush":
      return HasAmbush($cardID, $player, $index, "");
    case "Coordinate":
      return HasCoordinate($cardID, $player, $index);
    case "Exploit":
      return ExploitAmount($cardID, $player, true) > 0;
    case "Piloting":
      return PilotingCost($cardID) > -1;//TODO: test Boba Daimyo trigger when piloting as upgrade vs unit
    case "Any":
      return SmuggleCost($cardID, $player, $index) > -1 ||
...
{
  $character = &GetPlayerCharacter($player);
  if ($index >= count($character) || $index < 0)
    return false;
  return $character[$index + 1] == 2;
}
...
function DestroyCharacter($player, $index)
{
  ...
  $cardID = $char[$index];
  if ($char[$index + 6] == 1)
    RemoveCombatChain(GetCombatChainIndex($cardID, $player));
  $char[$index + 6] = 0;
...
function HaveUnblockedEquip($player)
{
  ...
  for ($i = CharacterPieces(); $i < count($char); $i += CharacterPieces()) {
    if ($char[$i + 1] == 0)
      continue;//If broken
    if ($char[$i + 6] == 1)
      continue;//On combat chain
    if (CardType($char[$i]) != "E")
      continue;
    if (BlockValue($char[$i]) == -1)
      continue;
    return true;
  }
  return false;
...
function NumEquipBlock()
{
  ...
  $numEquipBlock = 0;
  for ($i = CombatChainPieces(); $i < count($combatChain); $i += CombatChainPieces()) {
    if (CardType($combatChain[$i]) == "E" && $combatChain[$i + 1] == $defPlayer)
      ++$numEquipBlock;
  }
...
function CanPassPhase($phase)
{
  ...
  switch ($phase) {
    ...
    case "MULTICHOOSETEXT":
      return 0; // Deprecated, use CHOOSEOPTION instead
    case "CHOOSEOPTION":
      return 0;
    case "CHOOSEMYSOUL":
      return 0;
...
function AttackerMZID($player)
{
  global $combatChainState, $CCS_WeaponIndex, $mainPlayer;
  if ($player == $mainPlayer)
    return "MYALLY-" . $combatChainState[$CCS_WeaponIndex];
  else
    return "THEIRALLY-" . $combatChainState[$CCS_WeaponIndex];
}
...
function IsSpecificAuraAttacking($player, $index)
{
  ...
  if ($weaponIndex != $index)
    return false;
  if (!DelimStringContains(CardSubtype($combatChain[0]), "Aura"))
    return false;
  return true;
}
...
function SameWeaponEquippedTwice()
{
  ...
  if (count($weaponIndex) > 1 && $char[$weaponIndex[0]] == $char[$weaponIndex[1]])
    return true;
  return false;
}
...
function SelfCostModifier($cardID, $from, $reportMode = false)
{
      ...
      switch ($cardID) {
        case "6421006753"://The Mandalorian
          if ($reportMode)
            ...
            $cardAspects = GetClassState($currentPlayer, $CS_PlayedAsUpgrade) == "1" ? "Cunning" : "Cunning,Cunning";
          break;
      }
    }
    if ($cardAspects != "") {
        ...
        if ($playerAspects[$aspectArr[$i]] < 0) {
          ...
          switch ($cardID) {
            case "6263178121"://Kylo Ren (Killing the Past)
              if ($aspectArr[$i] != "Villainy" || !ControlsNamedCard($currentPlayer, "Rey"))
                ++$penalty;
              break;
            case "0196346374"://Rey (Keeping the Past)
              if ($aspectArr[$i] != "Heroism" || !ControlsNamedCard($currentPlayer, "Kylo Ren"))
                ++$penalty;
              break;
            default:
              ++$penalty;
  ...
  switch ($cardID) {
    ...
    case "7884488904"://For The Republic
      if (SearchCount(SearchAllies($currentPlayer, trait: "Republic")) >= 3)
        $modifier -= 2;
      break;
    case "4111616117"://Volunteer Soldier
      if (SearchCount(SearchAllies($currentPlayer, trait: "Trooper")) > 0)
        $modifier -= 1;
      break;
    ...
    case "8380936981"://Jabba's Rancor
      if (ControlsNamedCard($currentPlayer, "Jabba the Hutt"))
        $modifier -= 1;
      break;
    ...
    case "2443835595"://Republic Attack Pod
      if (SearchCount(SearchAllies($currentPlayer)) >= 3)
        $modifier -= 1;
      break;
    case "1356826899"://Home One
      $otherPlayer = $currentPlayer == 1 ? 2 : 1;
      if (SearchCount(SearchAllies($otherPlayer, arena: "Space")) >= 3)
        $modifier -= 3;
      break;
    default:
      break;
  ...
  if (count($layers) > 0) {
    ...
    if (SearchAlliesForCard($currentPlayer, "4166047484") != "")
      $targetID = "4166047484";
    else if (SearchAlliesForCard($currentPlayer, "fb7af4616c") != "")
      $targetID = "fb7af4616c";
    else if (SearchAlliesForCard($currentPlayer, "4776553531") != "")
      $targetID = "4776553531";
    else if ($cardID == "3141660491")
      $targetID = "4088c46c4d";
    else
      $targetID = "";
  }
  if (DefinedTypesContains($cardID, "Upgrade", $currentPlayer)) {
    if ($targetID == "4166047484")
      $modifier -= 1;//Guardian of the Whills
    if ($cardID == "0875550518" && ($targetID == "fb7af4616c" || $targetID == "4776553531"))
      $modifier -= 2;//Grievous's Wheel Bike
  ...
  for ($i = 0; $i < count($allies); $i += AllyPieces()) {
    ...
    switch ($allies[$i]) {
      case "5035052619"://Jabba the Hutt
        if (DefinedTypesContains($cardID, "Event", $currentPlayer) && TraitContains($cardID, "Trick", $currentPlayer))
          $modifier -= 1;
        break;
      //Jump to Lightspeed
      case "649c6a9dbd"://Admiral Piett
        if (TraitContains($cardID, "Capital Ship", $currentPlayer))
          $modifier -= 2;
        break;
      case "6311662442"://Director Krennic
        if (GetClassState($currentPlayer, $CS_NumWhenDefeatedPlayed) == 0 && HasWhenDestroyed($cardID))
          $modifier -= 1;
        break;
      default:
        break;
...
function IsCardNamed($player, $cardID, $name)
{
  ...
  for ($i = 0; $i < count($currentTurnEffects); $i += CurrentTurnEffectPieces()) {
    ...
    if ($name != "" && $currentTurnEffects[$i + 1] == $player)
      return true;
  }
  return false;
}
...
function HasAttackName($name)
{
  ...
  for ($i = 0; $i < count($chainLinkSummary); $i += ChainLinkSummaryPieces()) {
    ...
    for ($j = 0; $j < count($names); ++$j) {
      if ($name == GamestateUnsanitize($names[$j]))
        return true;
    }
  }
  return false;
...
function HasPlayedAttackReaction()
{
  ...
  for ($i = CombatChainPieces(); $i < count($combatChain); $i += CombatChainPieces()) {
    if ($combatChain[$i + 1] != $mainPlayer)
      continue;
    if (CardType($combatChain[$i]) == "AR" || GetResolvedAbilityType($combatChain[$i]))
      return true;
  }
  return false;
}
...
{
  if (file_exists("./Games/" . $gameName . "/gamestateBackup.txt"))
    unlink("./Games/" . $gameName . "/gamestateBackup.txt");
  if (file_exists("./Games/" . $gameName . "/beginTurnGamestate.txt"))
    unlink("./Games/" . $gameName . "/beginTurnGamestate.txt");
  if (file_exists("./Games/" . $gameName . "/lastTurnGamestate.txt"))
...
function PlayAbility($cardID, $from, $resourcesPaid, $target = "-", $additionalCosts = "-", $theirCard = false, $uniqueId = "")
{
  ...
  switch ($cardID) {
    ...
    case "3377409249"://Rogue Squadron Skirmisher
      if ($from != "PLAY")
        MZMoveCard($currentPlayer, "MYDISCARD:maxCost=2;definedType=Unit", "MYHAND", may: true);
      break;
    ...
    case "6702266551"://Smoke and Cinders
      $hand = &GetHand(1);
      for ($i = 0; $i < (count($hand) / HandPieces()) - 2; ++$i)
        PummelHit(1);
      $hand = &GetHand(2);
      for ($i = 0; $i < (count($hand) / HandPieces()) - 2; ++$i)
        PummelHit(2);
    ...
    case "3684950815"://Bounty Hunter Crew
      if ($from != "PLAY")
        MZMoveCard($currentPlayer, "MYDISCARD:definedType=Event", "MYHAND", may: true, context: "Choose an event to return with " . CardLink("3684950815", "3684950815"));
      break;
    ...
    case "3187874229"://Cassian Andor
      ...
      if ($abilityName == "Draw Card") {
        ...
        $otherPlayer = $currentPlayer == 1 ? 2 : 1;
        if (GetClassState($otherPlayer, $CS_DamageTaken) >= 3)
          Draw($currentPlayer);
      }
      break;
    ...
    case "4300219753"://Fett's Firespray
      if ($from != "PLAY") {
        $ready = false;
        if (ControlsNamedCard($currentPlayer, "Boba Fett") || ControlsNamedCard($currentPlayer, "Jango Fett"))
          $ready = true;
        if ($ready) {
          $playAlly->Ready();
        }
    ...
    case "2668056720"://Disabling Fang Fighter
      if ($from != "PLAY")
        DefeatUpgrade($currentPlayer, true);
      break;
    ...
    case "0505904136"://Scanning Officer
      ...
      foreach ($randomIndices as $randomIndex) {
        ...
        if (SmuggleCost($resources[$index], $otherPlayer, $index) >= 0) {
          ...
          for ($j = $index; $j < $index + ResourcePieces(); ++$j)
            unset($resources[$j]);
    ...
    case "5778949819"://Relentless Pursuit
      $ally = new Ally($target, $currentPlayer);
      if (TraitContains($ally->CardID(), "Bounty Hunter", $currentPlayer))
        $ally->Attach("8752877738");//Shield Token
    ...
    case "1743599390"://Trandoshan Hunters
      $otherPlayer = $currentPlayer == 1 ? 2 : 1;
      if (SearchCount(SearchAllies($otherPlayer, hasBountyOnly: true)) > 0)
        $playAlly->Attach("2007868442");//Experience token
      break;
    ...
    case "8929774056"://Asajj Ventress
      global $CS_NumEventsPlayed;
      if (GetClassState($currentPlayer, $CS_NumEventsPlayed) > 0)
        AddCurrentTurnEffect("8929774056", $currentPlayer, "PLAY");
      AddDecisionQueue("MULTIZONEINDICES", $currentPlayer, "MYALLY");
      AddDecisionQueue("MZFILTER", $currentPlayer, "status=1", 1);
      AddDecisionQueue("SETDQCONTEXT", $currentPlayer, "Choose a unit to attack with");
      AddDecisionQueue("CHOOSEMULTIZONE", $currentPlayer, "<-", 1);
    ...
    case "8345985976"://Trade Federation Shuttle
      if (SearchCount(SearchAllies($currentPlayer, damagedOnly: true)))
        CreateBattleDroid($currentPlayer);
      break;
...
function DamagePlayerAllies($player, $damage, $source, $type, $arena = "")
{
  ...
  for ($i = count($allies) - AllyPieces(); $i >= 0; $i -= AllyPieces()) {
    if ($arena != "" && !ArenaContains($allies[$i], $arena, $player))
      continue;
    $ally = new Ally("MYALLY-" . $i, $player);
...
function DamageAllAllies($amount, $source, $alsoRest = false, $alsoFreeze = false, $arena = "", $except = "")
{
  ...
  $theirAllies = &GetAllies($otherPlayer);
  for ($i = count($theirAllies) - AllyPieces(); $i >= 0; $i -= AllyPieces()) {
    if (!ArenaContains($theirAllies[$i], $arena, $otherPlayer))
      continue;
    if ($alsoRest)
      $theirAllies[$i + 1] = 1;
    if ($alsoFreeze)
  ...
  $allies = &GetAllies($currentPlayer);
  for ($i = count($allies) - AllyPieces(); $i >= 0; $i -= AllyPieces()) {
    if (!ArenaContains($allies[$i], $arena, $currentPlayer))
      continue;
    if ($except != "" && $except == ("MYALLY-" . $i))
      continue;
    if ($alsoRest)
      $allies[$i + 1] = 1;
    if ($alsoFreeze)
...
function DrawIntoMemory($player)
{
  $deck = &GetDeck($player);
  if (count($deck) > 0)
    AddMemory(array_shift($deck), $player, "DECK", "DOWN");
}
...
function PlayRequiresTarget($cardID)
{
  ...
  switch ($cardID) {
    case "8679831560":
      return 2;//Repair
    case "8981523525":
      return 6;//Moment of Peace
    case "0867878280":
      return 6;//It Binds All Things
    case "2587711125":
      return 6;//Disarm
    case "6515891401":
      return 7;//Karabast
    case "2651321164":
      return 6;//Tactical Advantage
    case "1900571801":
      return 7;//Overwhelming Barrage
    case "5013139687":
      return 3;//Caught In The Crossfire
    case "7861932582":
      return 6;//The Force is With Me
    case "2758597010":
      return 6;//Maximum Firepower
    case "2202839291":
      return 6;//Don't Get Cocky
    case "1701265931":
      return 6;//Moment of Glory
    case "3765912000":
      return 7;//Take Captive
    case "5778949819":
      return 7;//Relentless Pursuit
    case "1973545191":
      return 6;//Unexpected Escape
    case "0598830553":
      return 6;//Dryden Vos
    case "8576088385":
      return 6;//Detention Block Rescue
    default:
      return -1;
  }
}
...
function GetArcaneTargetIndices($player, $target)
{
  ...
  for ($i = count($targets) - 1; $i >= 0; --$i) {
    if (DelimStringContains($targetsSelected, $targets[$i]))
      unset($targets[$i]);
  }
...
function Draw($player, $mainPhase = true, $fromCardEffect = true)
{
  ...
  if (CurrentEffectPreventsDraw($player, $mainPhase))
    return -1;
