  ...
  if(!$isBlock) {
    ...
    if(!$chainClosed) {
      ...
      if(DefinedTypesContains($cardID, "Event", $currentPlayer) && SearchCurrentTurnEffects("3401690666", $currentPlayer, remove:true)) {
        ...
        if($from == "PLAY" || $from == "EQUIP" || HasWhenPlayed($cardID)) AddLayer("PLAYABILITY", $currentPlayer, $cardID, $from . "!" . $resourcesPaid . "!" . $target . "!" . $additionalCosts . "!" . $abilityIndex . "!" . $playIndex, "-", $uniqueID);
      }
    }
    if($from != "PLAY") {
